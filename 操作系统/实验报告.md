...
<img src="https://wwhds-markdown-image.oss-cn-beijing.aliyuncs.com/image-20241203144527402.png" alt="image-20241203144527402" style="zoom:50%;" />
...
## 六、调试时遇到的问题及解决方法
我使用的是Mac电脑，直接执行编译得到的产物无法在x86架构的linux上运行么，使用GO语言的交叉编译指令编译得到的产物即可

```bash
GOOS=linux GOARCH=amd64 go build -o process-scheduling process-scheduling.go
```
...
func shortestJobFirstScheduling(processes *[]Process) { // 实现短作业优先调度算法
	// 解引用指针以获取切片
	slice := *processes
...
func priorityScheduling(processes *[]Process) { // 实现优先权调度算法
	// 解引用指针以获取切片
	slice := *processes
